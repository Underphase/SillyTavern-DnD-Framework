...
try{
    ...
    await page.getByText('Дополнительно: HTML, CSS и ID',{exact:true}).click();
    await page.locator('[name="autoLayout"]').uncheck();
    ...
    await page.getByRole('button',{name:'JSON — дополнительно',exact:true}).click();
    ...
    await page.getByRole('button',{name:'Копировать отчёт',exact:true}).click();
    ...
    await page.waitForFunction(()=>/Ожидание ответа ИИ.*[1-9][0-9]* с/.test(document.querySelector('.rpg-status').textContent));
    assert.equal(await page.locator('.rpg-status').getAttribute('data-kind'),'working');
    assert.match(await page.locator('.rpg-status').textContent(),/Ожидание ответа ИИ.*[1-9][0-9]* с/);
    ...
    // New-chat setup is optional and uses forms, not JSON.
    assert.equal(fresh.setup,'pending');
    await page.getByRole('button',{name:'Настроить персонажа',exact:true}).click();
    await page.locator('[data-character-field="name"] textarea').fill('Рин');
    ...
    await skills.locator(':scope > .rpg-value-add [data-new-type]').selectOption('object');
    await skills.locator(':scope > .rpg-value-add button').click();
    const skill=skills.locator('[data-key="moonstep"] > .rpg-value');
    await skill.locator(':scope > .rpg-value-add [data-new-key]').fill('bonus');
    await skill.locator(':scope > .rpg-value-add [data-new-type]').selectOption('number');
    await skill.locator(':scope > .rpg-value-add button').click();
    await skill.locator('[data-key="bonus"] input').fill('5');
    await page.screenshot({path:resolve(root,'test-results/character-form-mobile.png')});
    assert.equal(await page.locator('.rpg-body').evaluate(el=>el.scrollWidth>el.clientWidth+1),false);
    await page.getByRole('button',{name:'Сохранить персонажа',exact:true}).click();
    const seeded=await page.evaluate(()=>fixture.context.chatMetadata.underphase_dnd);
    assert.equal(seeded.setup,'done');assert.equal(seeded.protagonist,null);
    assert.equal(seeded.characters[0].level,8);assert.equal(seeded.characters[0].experience_target,2000);
    assert.equal(seeded.characters[0].skills.moonstep.bonus,5);
    assert.ok(await page.evaluate(()=>fixture.prompts.underphase_dnd.includes('"level":8')));
    assert.ok(await page.evaluate(()=>fixture.prompts.underphase_dnd.includes('NEVER character knowledge')));
    assert.match(await page.locator('.rpg-body').textContent(),/1200 \/ 2000 XP/);
    await page.locator('[data-action="edit-character"]').click();
    await page.locator('[data-character-field="level"] input').fill('9');
    await page.getByRole('button',{name:'Сохранить персонажа',exact:true}).click();
    assert.equal(await page.evaluate(()=>fixture.context.chatMetadata.underphase_dnd.characters[0].skills.moonstep.bonus),5);
    // Visual tracker fields preserve values and per-field instructions, with generated HTML.
    await page.getByRole('button',{name:'Мастерская',exact:true}).click();
    await page.locator('[name="trackerName"]').fill('Доверие спутника');
    const firstField=page.locator('.rpg-tracker-field').first();
    await firstField.locator('[data-field="label"]').fill('Доверие');
    await firstField.locator('[data-field="instruction"]').fill('Менять только после поступков, влияющих на доверие.');
    await page.getByRole('button',{name:'＋ Добавить показатель',exact:true}).click();
    assert.equal(await page.locator('.rpg-tracker-field').count(),2);
    await page.locator('.rpg-tracker-field').last().getByRole('button',{name:'Удалить',exact:true}).click();
    await page.getByRole('button',{name:'Обновить пример',exact:true}).click();
    await page.screenshot({path:resolve(root,'test-results/tracker-form-mobile.png')});
    assert.equal(await page.locator('.rpg-body').evaluate(el=>el.scrollWidth>el.clientWidth+1),false);
    await page.getByRole('button',{name:'Сохранить',exact:true}).click();
    ...
    assert.equal(await page.evaluate(()=>fixture.context.chatMetadata.underphase_dnd.characters.length),0);
