...
test('skills and active abilities with matching keys remain separate',()=>{
    ...
    assert.match(html,/Бонус: 0/);assert.match(html,/Стоимость: 2/);assert.doesNotMatch(html,/<pre>/);
});
test('free-form lists, nested mechanics and long descriptions stay accessible and escaped',()=>{
    ...
    assert.match(html,/&lt;script&gt;/);assert.match(html,/&lt;img src=x&gt;/);assert.match(html,/Подробнее/);
