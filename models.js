...
export async function listModels(address, key, { signal } = {}) {
    ...
    if (!response.ok) {
        throw new Error(`List models: HTTP ${response.status}. ${response.status === 401 || response.status === 403 ? 'Проверь ключ ИИ и доступ к провайдеру.' : 'Проверь адрес API и поддержку GET /models.'}`);
    }
