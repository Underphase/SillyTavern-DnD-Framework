...
export class FrameworkUI {
    constructor(actions) {
        this.actions=actions;this.statusTracker=new StatusTracker();this.reportedErrors=new WeakSet();this.tab='story';this.draft=trackerTemplate();this.draftOriginal=null;
    ...
    async runAction(control){
        ...
        const labels={'suggest-focus':'Выбор главного героя','generate-tracker':'Создание треккера','test-ai':'Проверка подключения ИИ','test-backend':'Проверка старого API','import-character':'Импорт персонажа','load-models':'Загрузка моделей','openrouter-models':'Загрузка моделей','save-character':'Сохранение персонажа','save-memory':'Сохранение памяти'};
        ...
        try{await this.handle(action,control);this.finishTask(task,this.statusTracker.last===previous?`${labels[action]} — готово`:this.statusTracker.last.message,this.statusTracker.last===previous?'success':this.statusTracker.last.kind);}
        catch(error){if(!this.statusTracker.tasks.has(task))return;this.finishTask(task,error.message,'error');throw error;}
    }
    refresh(){if(this.dialog.open && !['workshop','settings'].includes(this.tab) && !this.dialog.querySelector('.rpg-character-editor:not([hidden])') && !this.dialog.contains(document.activeElement?.closest('textarea,input,select')))this.render();}
    render(){
    ...
    content(s,p){
        ...
        return `<h3>Диагностика</h3><div class="rpg-toolbar">${button('copy-report','Копировать отчёт')}${button('download-report','Скачать отчёт')}</div><p class="rpg-muted">Ключи скрыты. Отчёт Dev может содержать фрагменты истории — проверь его перед отправкой.</p><textarea class="rpg-code" id="rpg-report-fallback" aria-label="Отчёт об ошибках" hidden readonly></textarea><p class="rpg-muted">Расход токенов показывается, если его вернул провайдер. Символы — не токены.</p>${s.activity.slice(-60).reverse().map(a=>details(`${formatTimestamp(a.at)} · ${a.title}`,a.data)).join('')||empty('Журнал пуст','Здесь появятся время запросов, расход токенов и ошибки.')} ${button('clear-log','Очистить журнал')}`;
    }
    story(s){
        const focus=s.protagonist?.name??'Ансамбль персонажей';
        return `${s.setup==='pending'?`<section class="rpg-setup-card"><h3>Перед началом истории</h3><p>Хочешь заранее задать персонажа, уровень, опыт и навыки? Эти данные будут доступны ИИ с самого начала. Выбор главного героя — отдельно.</p>${button('setup-character','Настроить персонажа','class="rpg-primary"')} ${button('skip-setup','Пропустить')}</section>`:''}<section class="rpg-hero"><div><div class="rpg-eyebrow">В ЦЕНТРЕ ИСТОРИИ</div><h1>${e(focus)}</h1><p>${s.protagonist?'Фокус на герое. Сцена развивается постепенно, с сохранением ролей участников.':'Никто не назначен главным героем. Фокус разделён между участниками текущей сцены.'}</p></div><span class="rpg-crescent">☾</span></section><div class="rpg-toolbar">${button('focus','✦ Выбрать главного героя')}${button('suggest-focus','Предложение ИИ')}${button('tab','＋ Создать треккер','data-tab="workshop"')}</div>
    ...
    character(c,s,compact=false){
        ...
        ${on.progress?`<div class="rpg-chips"><span>Уровень ${e(c.level)}</span><span>${e(c.experience)} / ${c.experience_target>0?e(c.experience_target):'—'} XP</span></div>${c.experience_target>0?`<progress class="rpg-xp" aria-label="Опыт до следующего уровня" max="${e(c.experience_target)}" value="${Math.min(c.experience_target,Math.max(0,c.experience))}"></progress><small class="rpg-muted">Осталось ${Math.max(0,c.experience_target-c.experience)} XP</small>`:'<small class="rpg-muted">Порог опыта можно задать в редакторе.</small>'}`:''}
    ...
    characters(s){return `<div class="rpg-section-heading"><div><h3>Characters in this story</h3><p class="rpg-muted">Персонажи сохраняются в текущем чате SillyTavern.</p></div><div>${button('new-character','＋ Персонаж')} ${button('refresh-characters','↻ Обновить список')}</div></div><div class="rpg-character-editor" hidden></div><div class="rpg-character-grid">${s.characters.map(c=>this.character(c,s)).join('')}</div><details class="rpg-details"><summary>Импортировать персонажа из старого API</summary><p>Создаёт отдельную копию в этом чате.</p>${field('importId','owner_id существующего персонажа','')}${button('import-character','Импортировать копию')}</details>`;}
    workshop(s){const t=this.draft;return `<div class="rpg-section-heading"><div><h3>Свой треккер</h3><p class="rpg-muted">Назови показатель, выбери его вид и опиши правило обновления. Код писать не нужно.</p></div>${button('new-tracker','Новый')}</div><div class="rpg-workshop-grid"><div data-workshop><div class="rpg-toolbar">${[['meter','Шкала'],['number','Счётчик'],['inventory','Список'],['text','Заметка']].map(([type,label])=>button('tracker-preset',label,`data-type="${type}"`)).join('')}</div>${area('builderRequest','Что нужно отслеживать?',this.builderRequest??'','placeholder="Например: доверие спутника от 0 до 100, меняется после поступков"')}${button('generate-tracker','✦ Создать с ИИ','class="rpg-primary"')}${field('trackerName','Название',t.name)}${area('trackerPrompt','Общее правило для ИИ',t.prompt)}<div id="rpg-tracker-field-list">${trackerFieldsEditor(t.fields)}</div>${button('add-tracker-field','＋ Добавить показатель')}<label class="rpg-check"><input type="checkbox" name="autoLayout" ${this.autoLayout!==false?'checked':''}> Автоматическое оформление</label><details class="rpg-details"><summary>Дополнительно: HTML, CSS и ID</summary>${field('trackerId','ID',t.id)}${area('trackerHtml','HTML · {{id}} вставляет показатель',t.html,'class="rpg-code"')}${area('trackerCss','CSS',t.css,'class="rpg-code"')}<p class="rpg-muted">Для собственного HTML отключи автоматическое оформление.</p></details><div class="rpg-toolbar">${button('preview','Обновить пример')}${button('save-tracker','Сохранить','class="rpg-primary"')}${button('cancel-tracker','Отмена')}</div></div><aside class="rpg-preview"><div class="rpg-eyebrow">ПРИМЕР · ВЫМЫШЛЕННЫЕ ДАННЫЕ</div><iframe sandbox="" referrerpolicy="no-referrer" title="Предпросмотр треккера" id="rpg-tracker-preview"></iframe><div class="rpg-muted" id="rpg-widget-keys"></div></aside></div>${s.trackers.length?`<h3>Сохранённые треккеры</h3>${s.trackers.map(t=>`<div class="rpg-saved-tracker"><span>${e(t.name)}</span><div>${button('edit-tracker','Изменить',`data-id="${e(t.id)}"`)} ${button('delete-tracker','Удалить',`data-id="${e(t.id)}"`)}</div></div>`).join('')}`:''}`;}
    openCharacter(data,id=null,setup=false,revision){
        this.editingId=id;this.editingRevision=revision;this.editingSetup=setup;this.characterDraft=data;this.rawCharacter=false;this.tab='characters';this.render();this.renderCharacterEditor();
    }
    renderCharacterEditor(){
        const editor=this.dialog.querySelector('.rpg-character-editor');editor.hidden=false;
        editor.innerHTML=`<h3>${this.editingSetup?'Персонаж перед началом игры':'Карточка персонажа'}</h3>${this.rawCharacter?area('characterJson','JSON — дополнительный режим',json(this.characterDraft),'class="rpg-code"'):characterEditor(this.characterDraft)}${this.editingSetup?'<label class="rpg-check"><input name="setupFocus" type="checkbox"> Сделать главным героем</label>':''}<div class="rpg-toolbar">${button('save-character','Сохранить персонажа','class="rpg-primary"')}${button('toggle-character-json',this.rawCharacter?'Обычная форма':'JSON — дополнительно')}${button('tab','Отмена','data-tab="story"')}</div>`;
    }
    ...
    async handle(action,control){
        ...
        if(action==='add-tracker-field'||action==='remove-tracker-field'){
            this.readDraft();
            if(action==='remove-tracker-field'&&this.draft.fields.length===1)throw new Error('Оставь хотя бы один показатель');
            if(action==='remove-tracker-field')this.draft.fields.splice(Number(control.dataset.index),1);
            else{let n=1;while(this.draft.fields.some(f=>f.id===`field_${n}`))n++;this.draft.fields.push({id:`field_${n}`,label:'Новый показатель',type:'meter',max:100,value:50,source:'ai',instruction:''});}
            this.render();return;
        }
        ...
        if(action==='suggest-focus'){
            ...
            this.status(`Предложение: ${result.name}. ${result.reason??''} Нажми «Выбрать» для подтверждения.`,'warning');return;
        }
        ...
        if(action==='generate-tracker'){
            ...
            this.draft=draft;this.autoLayout=false;this.draftOriginal=null;this.render();this.status('Проверь пример и правила. Сохранение — только по кнопке.','warning');return;
        }
        ...
        if(action==='new-character'||action==='edit-character'){
            ...
            const data=c?{...characterDefaults(),experience_target:0,...Object.fromEntries(Object.entries(c).filter(([k])=>!['id','owner_id','scope_id','created_at','updated_at'].includes(k)))}:{...characterDefaults(),name:'Новый персонаж'};
        ...
        if(action==='save-character'){
            ...
            this.editingSetup=false;this.status('Персонаж сохранён');this.render();return;
        }
        ...
        if(action==='save-settings'||action==='test-ai'||action==='test-backend'){
            const p=this.actions.settings();for(const key of ['backendUrl','backendKey','aiMode','profileId','aiUrl','aiKey','model','narratorPrompt','scenePrompt','dicePrompt','processorPrompt','builderPrompt'])p[key]=this.value(key);
        ...
        if(action==='copy-report'||action==='download-report'){
            ...
            if(action==='download-report'){
                ...
                this.status('Отчёт скачан');return;
            }
            try{await navigator.clipboard.writeText(text);this.status('Отчёт скопирован');}
            catch{const field=this.dialog.querySelector('#rpg-report-fallback');field.hidden=false;field.value=text;field.focus();field.select();this.status('Буфер обмена недоступен. Выдели и скопируй отчёт ниже.','warning');}
            return;
        }
