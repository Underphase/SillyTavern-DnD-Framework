...
export class FrameworkUI {
    ...
    story(s){
        ...
        return `<section class="rpg-hero"><div><div class="rpg-eyebrow">В ЦЕНТРЕ ИСТОРИИ</div><h1>${e(focus)}</h1><p>${s.protagonist?'Персонажи продолжают жить и действовать самостоятельно.':'Никто не назначен главным героем. Мир не ждёт пользователя.'}</p></div><span class="rpg-crescent">☾</span></section><div class="rpg-toolbar">${button('focus','✦ Выбрать главного героя')}${button('suggest-focus','Предложение ИИ')}${button('tab','＋ Создать треккер','data-tab="workshop"')}</div>
        ...
        ${s.characters.length?`<div class="rpg-character-grid">${s.characters.map(c=>this.character(c,s,true)).join('')}</div>`:empty('История ещё не записана','Настрой отдельную ИИ и адрес RPG API. Персонажи появятся после обработки сообщений.')}
    ...
    async handle(action,control){
        ...
        if(action==='new-character'||action==='edit-character'){
            ...
            const editor=this.dialog.querySelector('.rpg-character-editor');editor.hidden=false;editor.innerHTML=`${area('characterJson','Данные персонажа (JSON)',json(data),'class="rpg-code"')}${button('save-character','Сохранить в API','class="rpg-primary"')} ${button('tab','Отмена','data-tab="characters"')}`;return;
        }
        ...
        if(action==='save-settings'||action==='test-ai'||action==='test-backend'){
            const p=this.actions.settings();for(const key of ['backendUrl','backendKey','aiMode','profileId','aiUrl','aiKey','model','narratorPrompt','processorPrompt','builderPrompt'])p[key]=this.value(key);
