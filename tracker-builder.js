...
export function readTrackerFields(root,original){return [...root.querySelectorAll('.rpg-tracker-field')].map(node=>{
    ...
    if(['number','meter','coins','gems'].includes(result.type)){if(!Number.isFinite(Number(result.value)))throw new Error('Пример числового показателя должен быть числом');result.value=Number(result.value);}
    if(result.type==='inventory'&&!Array.isArray(result.value)&&typeof result.value==='string')result.value=result.value.split('\n').filter(Boolean);
    if(result.type==='boolean'&&typeof result.value!=='boolean'){if(!['true','false','да','нет'].includes(String(result.value).toLowerCase()))throw new Error('Пример: true / false или да / нет');result.value=['true','да'].includes(String(result.value).toLowerCase());}
    return result;
});}
export function autoTrackerHtml(name,fields){return `<h2>${e(name)}</h2><div class="grid">${fields.map(f=>`{{${f.id}}}`).join('')}</div>`;}

export const sampleForType=type=>type==='inventory'?['Предмет']:type==='boolean'?false:['number','meter','coins','gems'].includes(type)?50:'Текст';
export function trackerTemplate(type='meter'){
    const name={meter:'Шкала',number:'Счётчик',inventory:'Список вещей',coins:'Монеты',gems:'Самоцветы',boolean:'Состояние',text:'Заметка'}[type]??'Треккер';
    const fields=[{id:'value',label:name,type,max:100,value:sampleForType(type),source:'ai',instruction:''}];
    return {id:`tracker_${Date.now()}`,name,prompt:'Меняй значение только по подтверждённым событиям истории.',fields,html:autoTrackerHtml(name,fields),css:''};
}
