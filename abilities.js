...
const text=value=>value===null||value===undefined?'—':typeof value==='boolean'?(value?'Да':'Нет'):String(value);
...
function abilityCard(entry,group,key){
    ...
    const name=data.name??(entry.key.startsWith('item-')?(typeof entry.value==='string'?entry.value:'Способность'):title(entry.key));
    ...
    const badges=[];
    if(data.passive===true||data.type==='passive')badges.push('Пассивный');
...
export function renderAbilities(character,scope=''){
    const groups=[['skills','Навыки','skill'],['active_skills','Активные способности','active']].map(([field,label,type])=>({field,label,type,...abilityEntries(character[field])}));
    ...
    return `<details class="rpg-details rpg-abilities" data-rpg-disclosure="${e(key)}"><summary>Навыки и способности${count?` <span class="rpg-ability-count">${count}</span>`:''}</summary>${body||'<p class="rpg-muted rpg-ability-empty">Пока нет записанных способностей.</p>'}</details>`;
}
