...
const labels={bonus:'Bonus',level:'Level',rank:'Rank',cost:'Cost',cooldown:'Cooldown',duration:'Duration',range:'Range',uses:'Uses',charges:'Charges',max:'Maximum',current:'Current',effect:'Effect',effects:'Effects',requirements:'Requirements',condition:'Condition',conditions:'Conditions',automatic_success:'Guaranteed success',actions:'Actions',passive:'Passive',type:'Type',status:'Status',description:'Description',items:'List'};
const title=key=>labels[key]??String(key).replace(/[_-]+/g,' ');
const text=value=>value===null||value===undefined?'—':typeof value==='boolean'?(value?'Yes':'No'):String(value);
...
function abilityCard(entry,group,key){
    ...
    const name=data.name??(entry.key.startsWith('item-')?(typeof entry.value==='string'?entry.value:'Ability'):title(entry.key));
    ...
    const badges=[];
    if(data.passive===true||data.type==='passive')badges.push('Passive');
    ...
    return `<article class="rpg-ability"><div class="rpg-ability-heading"><span class="rpg-ability-icon" aria-hidden="true">${group==='active'?'✧':'✦'}</span><h4>${e(name)}</h4></div>${badges.length?`<div class="rpg-ability-badges">${badges.map(b=>`<span>${e(b)}</span>`).join('')}</div>`:''}${description?`<p class="rpg-ability-description">${e(long?description.slice(0,180)+'…':description)}</p>`:''}${nonObject?`<div class="rpg-ability-value">${valueView(entry.value)}</div>`:''}${long||Object.keys(extra).length?`<details class="rpg-ability-more" data-rpg-disclosure="${e(key)}"><summary>Details</summary>${long?`<p>${e(description)}</p>`:''}${valueView(extra)}</details>`:''}</article>`;
}

export function renderAbilities(character,scope=''){
    const groups=[['skills','Skills','skill'],['active_skills','Active abilities','active']].map(([field,label,type])=>({field,label,type,...abilityEntries(character[field])}));
    ...
    return `<details class="rpg-details rpg-abilities" data-rpg-disclosure="${e(key)}"><summary>Skills and abilities${count?` <span class="rpg-ability-count">${count}</span>`:''}</summary>${body||'<p class="rpg-muted rpg-ability-empty">No abilities recorded yet.</p>'}</details>`;
}
