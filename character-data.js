...
export function normalizeCharacterData(data, existing = {}) {
    ...
    for(const [field, original] of Object.entries(data)) {
        ...
        if(OBJECT_FIELDS.includes(field)) {
            ...
            if(!Number.isSafeInteger(value)||value < -2147483648||value > 2147483647||(['experience','experience_target'].includes(field)&&value<0))throw new Error(`Character field ${field}: expected an integer${field==='experience'?' не меньше нуля':''}`);
