...
function stored(character,path){
    ...
    const value=pathValue(character,path);if(value===undefined)fail(`Нет сохранённого значения: ${path}`);return value;
}
function parameters(state,input,event){
    ...
    for(const [key,values] of Object.entries({comparison:['gte','lte'],mode:['normal','advantage','disadvantage'],attribute_rule:['none','raw','d20'],resolution:['roll','automatic','impossible']}))if(!values.includes(p[key]))fail(`Некорректный ${key}`);
