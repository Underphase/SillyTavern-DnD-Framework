...
function characterDefaults(){return {...Object.fromEntries(OBJECT_FIELDS.map(k=>[k,{}])),...Object.fromEntries(attributes.map(k=>[k,10])),description:'',background:'',tags:[],is_player:false,is_temporary:false,level:1,experience:0,armor_class:10,max_hp:10,current_hp:10,temporary_hp:0,general_condition:'Healthy'};}
export function syncLocal(state,pending){
    ...
    for(const change of pending.changes){
        ...
        if(!character){if(!data.name)fail('A new character needs a name');character={...characterDefaults(),owner_id:change.owner_id,scope_id:state.scopeId,created_at:new Date().toISOString()};characters.push(character);}
...
function stored(character,path){
    if(typeof path!=='string'||!roots.includes(path.split('.')[0]))fail('Unsupported stored attribute path');
...
function parameters(state,input,event){
    ...
    if(Object.keys(input).some(k=>!allowed.includes(k)))fail('Unknown check parameter');
    if(typeof p.check_key!=='string'||!p.check_key.trim()||p.check_key.length>150||typeof p.reason!=='string'||!p.reason.trim()||p.reason.length>2000)fail('A check_key and reason are required');
    if(typeof p.formula!=='string'||!/^([1-9]|[1-9][0-9]|100)d([2-9]|[1-9][0-9]{1,3}|10000)$/.test(p.formula))fail('Formula: 1–100 dice, each with 2–10000 sides');
    ...
    if(p.attribute!==null&&!attributes.includes(p.attribute))fail('Unknown attribute');
    if(!Array.isArray(p.bonus_paths)||p.bonus_paths.length>20||p.bonus_paths.some(v=>typeof v!=='string'||v.length>200)||new Set(p.bonus_paths).size!==p.bonus_paths.length)fail('Invalid or duplicate bonus paths');
    if(p.difficulty!==null&&(!integer(p.difficulty)||typeof p.difficulty_reason!=='string'||!p.difficulty_reason.trim()))fail('An integer difficulty and justification are required');
    if(p.difficulty_path&&(p.difficulty!==null||!p.target_id))fail('difficulty_path requires target_id; omit explicit difficulty');
    if(p.mode!=='normal'&&p.formula!=='1d20')fail('Advantage and disadvantage require 1d20');
    if((p.attribute||p.bonus_paths.length||p.passive_path)&&!p.actor_id)fail('Modifiers require actor_id');
    if(p.attribute_rule!=='none'&&!p.attribute)fail('An attribute is required');
    if(p.resolution==='automatic'&&!p.passive_path)fail('Automatic success requires a stored passive skill');
    return p;
}
...
export function resolveLocalCheck(state,input,event,rng=randomBelow){
    ...
    if(p.actor_id&&!actor||p.target_id&&!target)fail('Character not found in this chat');
    ...
    if(p.difficulty_path){
        ...
        if(!integer(difficulty))fail('Stored difficulty must be an integer');
        evidence.difficulty={target_id:p.target_id,path:p.difficulty_path,value:difficulty};
    }
    if(p.attribute&&p.attribute_rule!=='none'){
        const score=actor[p.attribute];if(!integer(score))fail('Invalid stored attribute value');
        modifier=p.attribute_rule==='raw'?score:Math.floor((score-10)/2);
        evidence[p.attribute]={score,rule:p.attribute_rule,bonus:modifier};
    }
    for(const path of p.bonus_paths){const value=stored(actor,path);if(!integer(value))fail('Bonus must be a stored integer');modifier+=value;evidence[path]=value;}
    let rolls=[],total=null,success;
    if(p.resolution==='automatic'){
        const passive=stored(actor,p.passive_path);
        if(passive?.automatic_success!==true||!Array.isArray(passive.actions)||!passive.actions.includes(p.check_key))fail('The passive does not grant automatic success for this action');
