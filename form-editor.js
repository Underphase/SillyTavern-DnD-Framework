...
const types='<option value="text">Текст</option><option value="number">Число</option><option value="boolean">Да / нет</option><option value="object">Группа полей</option><option value="array">Список</option>';
...
export function valueEditor(value){
    ...
    if(kind==='object'||kind==='array')return `<div class="rpg-value" data-kind="${kind}"><div class="rpg-value-rows">${Object.entries(value).map(([key,v])=>`<div class="rpg-value-row" data-key="${e(key)}"><div class="rpg-section-heading"><strong>${e(kind==='array'?Number(key)+1:labelFor(key))}</strong><button data-action="value-remove" type="button" aria-label="Удалить поле">×</button></div>${valueEditor(v)}</div>`).join('')}</div><div class="rpg-value-add">${kind==='object'?'<input data-new-key placeholder="Название нового поля" aria-label="Название нового поля">':''}<select data-new-type aria-label="Тип нового поля">${types}</select><button type="button" data-action="value-add">＋ Добавить</button></div></div>`;
    return `<div class="rpg-value" data-kind="${kind}">${kind==='boolean'?`<input type="checkbox" aria-label="Значение" ${value?'checked':''}>`:kind==='number'?`<input type="number" step="any" aria-label="Значение" value="${e(value)}">`:kind==='null'?'<span class="rpg-muted">Не задано</span>':`<textarea aria-label="Значение" rows="2">${e(value)}</textarea>`}</div>`;
}
export function readValue(node){
    ...
    if(kind==='number'){const input=node.querySelector('input');if(input.value.trim()===''||!Number.isFinite(Number(input.value)))throw new Error('Укажи корректное число');return Number(input.value);}
...
export function editValue(action,control){
    ...
    if(Array.isArray(data))data.push(value);
        ...
        if(!key||['__proto__','prototype','constructor'].includes(key)||Object.hasOwn(data,key))throw new Error('Название поля должно быть непустым и уникальным');
        data[key]=value;
    }
    node.outerHTML=valueEditor(data);
}

const groups=[['Основное',['name','level','experience','experience_target','is_player','is_temporary']],['Здоровье и характеристики',['current_hp','max_hp','temporary_hp','armor_class','general_condition','strength','dexterity','constitution','intelligence','wisdom','charisma']],['Личность и история',['description','background','race','character_class','personality','goals','traits']],['Навыки и вещи',['skills','active_skills','spells','spell_slots','inventory','equipment']],['Состояние и связи',['buffs','debuffs','current_state','relationships','memories','notes','tags']]];
export function characterEditor(data){
    return `<div class="rpg-sheet-editor">${groups.map(([title,keys],i)=>`<details class="rpg-editor-group" ${i===0?'open':''}><summary>${title}</summary><div class="rpg-editor-grid">${keys.filter(k=>Object.hasOwn(data,k)).map(k=>`<section class="rpg-edit-field" data-character-field="${k}"><label>${labelFor(k)}</label>${valueEditor(data[k])}</section>`).join('')}</div></details>`).join('')}<p class="rpg-muted">Опыт — накопленное значение. Порог следующего уровня задаётся по правилам мира (0 — не задан). Сохранение не повышает уровень автоматически.</p></div>`;
}
